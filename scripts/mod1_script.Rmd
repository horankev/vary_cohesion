...
```{r}
mod1_ld <- brm(ld24 ~ first_party19 + second_party19comp * majority_prop_scale +
               ...
               seed = 123,
               prior = priors,
               # prior = set_prior("", class = "Intercept"),  # No prior for the intercept
               save_pars = save_pars(all = TRUE))
saveRDS(mod1_ld, here("models5","mod1_ld.rds"))

# mod1_ld <- readRDS(here("models5","mod1_ld.rds"))

summary_mod1_ld <- rbind(summary(mod1_ld)$fixed,
                           summary(mod1_ld)$random$region_name) |>
  data.frame() |>
  rownames_to_column("par")

margloglik_mod1_ld <- bridge_sampler(mod1_ld)
saveRDS(margloglik_mod1_ld, here("models5","margloglik_mod1_ld.rds"))

# margloglik_mod1_ld <- readRDS(here("models5","margloglik_mod1_ld.rds"))
...
```{r}
mod1_con <- brm(con24 ~ first_party19 + second_party19comp * majority_prop_scale +
                  degree_scale + notgoodhealth_scale + white_scale + region_name +
                  offset(log(valid_votes24)),
                data=dfnbcon,
                family = poisson(),      # named list of data
                ...
                # prior = set_prior("", class = "Intercept"),  # No prior for the intercept
                save_pars = save_pars(all = TRUE))
saveRDS(mod1_con, here("models5","mod1_con.rds"))

# mod1_con <- readRDS(here("models5","mod1_con.rds"))

summary_mod1_con <- rbind(summary(mod1_con)$fixed,
                           summary(mod1_con)$random$region_name) |>
  data.frame() |>
  rownames_to_column("par")

margloglik_mod1_con <- bridge_sampler(mod1_con)
saveRDS(margloglik_mod1_con, here("models5","margloglik_mod1_con.rds"))

# margloglik_mod1_con <- readRDS(here("models5","margloglik_mod1_con.rds"))
...
```{r}
mod1_lab <- brm(lab24 ~ first_party19 + second_party19comp * majority_prop_scale +
                  degree_scale + notgoodhealth_scale + white_scale + region_name +
                  offset(log(valid_votes24)),
                data=dfnblab,
                family = poisson(),      # named list of data
                ...
                # prior = set_prior("", class = "Intercept"),  # No prior for the intercept
                save_pars = save_pars(all = TRUE))
saveRDS(mod1_lab, here("models5","mod1_lab.rds"))

mod1_lab <- readRDS(here("models5","mod1_lab.rds"))

summary_mod1_lab <- rbind(summary(mod1_lab)$fixed,
                           summary(mod1_lab)$random$region_name) |>
  data.frame() |>
  rownames_to_column("par")

margloglik_mod1_lab <- bridge_sampler(mod1_lab)
saveRDS(margloglik_mod1_lab, here("models5","margloglik_mod1_lab.rds"))

# margloglik_mod1_lab <- readRDS(here("models5","margloglik_mod1_lab.rds"))
...
```{r}
mod1_ruk <- brm(ruk24 ~ first_party19 + second_party19comp * majority_prop_scale +
                  degree_scale + notgoodhealth_scale + white_scale + region_name +
                  offset(log(valid_votes24)),
                data=dfnbruk,
                family = poisson(),      # named list of data
                ...
                # prior = set_prior("", class = "Intercept"),  # No prior for the intercept
                save_pars = save_pars(all = TRUE))
saveRDS(mod1_ruk, here("models5","mod1_ruk.rds"))

mod1_ruk <- readRDS(here("models5","mod1_ruk.rds"))

summary_mod1_ruk <- rbind(summary(mod1_ruk)$fixed,
                           summary(mod1_ruk)$random$region_name) |>
  data.frame() |>
  rownames_to_column("par")

margloglik_mod1_ruk <- bridge_sampler(mod1_ruk)
saveRDS(margloglik_mod1_ruk, here("models5","margloglik_mod1_ruk.rds"))

# margloglik_mod1_ruk <- readRDS(here("models5","margloglik_mod1_ruk.rds"))
